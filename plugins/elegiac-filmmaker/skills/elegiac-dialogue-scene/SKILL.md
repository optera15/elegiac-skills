...
4. **Lipsync.** `generate_lipsync` with `videoUrl` = the approved clip, `audioUrl` = the approved read. Default `syncMode` cut_off; pass `duration` matching the audio length so the quote is honest.
