...
8. **Develop recurring memory cheaply.** Use `add_visual_style_board`, `add_character_concept`, and `add_location_concept` for text-first mood, cast, location, and hero-prop development. Quote a small casting/look proof only when useful. Promote approved style artifacts so later episodes reuse IDs rather than copied prompt prose.
   - **Each principal becomes one Cast member with a true Consistent Character sheet**, in this order: (a) `add_character_concept` with name, role, backstory, wardrobe, personality, look notes; (b) `create_character_on_board` on the same board with that name and a description assembled from the card (one image approval; creates the Cast member and returns `castMemberId`); (c) `make_board_character_consistent` with the returned `boardItemId` (one image approval; the 7-panel identity sheet, marked `isConsistentCharacter`, attached to the same Cast member); (d) `promote_character_concept` with `castMemberId` set to that Cast member (link mode: fills the bible from the card, marks the card approved). Inspect the sheet: exactly 4 full-body rotations over 3 close-ups, nothing cropped; regenerate if not. A single portrait from an image model is not a Consistent Character.
   - **Never call `promote_character_concept` without `castMemberId` once a board character exists for that concept**: without it the tool creates a second Cast member and the show ends up with two entries for one character. Check `list_characters` before creating anything.
