...
| **Muse Image** (`muse-image`) | Cheapest reference-capable stills (flat 3 cr); accurate in-image text, plots and QR codes; 1–10 refs route to /edit | No seed or resolution control — Muse picks the size; no 4:5/5:4/2.35:1 |
...
**Default routing:** text-on-image → GPT Image 2. Character continuity → Nano Banana with cast refs. Stylized → Seedream 5.0 / Reve. Identity-locked edits of an existing still → Kling O1 Image or Nano Banana Pro. Drafts → Nano Banana 2 Lite, Grok Imagine Image at 1K, or Muse Image (cheapest with references; also a good low-cost pick for text-on-image drafts).
...
| **MiniMax H3** | Hero-tier 2K video with synchronized native audio (Hailuo-03): first/last keyframes, multimodal refs (9 images / 3 videos / 3 audio, cited as `@Image1`-style tokens). `qualityTier: "max"` runs H3 Max — the CHEAPER post-trained tier at 480p/768p with stronger prompt adherence | Premium priced (~325 cr/5s base; extra image refs beyond 5 and ref-video seconds add cost); 2K only on standard, 5-15s. H3 Max is ~63 cr/5s at 480p and ~100 at 768p on T2V/I2V, and a flat ~100 cr/5s on reference-to-video plus per-token reference charges; send `max` only on an explicit H3 Max ask, never for "max quality" language |
...
| **Gemini Omni Flash 1.1** (`gemini-omni-flash`) | 3-10s clip with generated audio at 360p-4K, from text, a start image (optional end frame), or up to 10 image + 3 short video references; also edits an existing video from a written instruction | 3-10s (default 8), 360p/720p/1080p/4k (default 720p), 16:9 or 9:16; `edit` takes `sourceVideoUrl` and neither duration nor aspect ratio — it bills the 10s ceiling and refunds down to the delivered length |
