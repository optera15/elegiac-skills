...
8. Reframe the proof shots first: `list_framings` → one `enhance_prompt` call with `shots[]` and the scene's references (see `craft.md`, Framing and prompt enhancement), then generate the proof with `generate_shots` using the enhanced prompts verbatim. Quote/cap a 3–6-frame still proof. Check identity, wardrobe, props, geography, screen direction, lighting, phone-size composition, hook, and cliffhanger. Curate or surgically repair before motion.
9. Produce the clips through `production-pipeline.md` unless the user overrides it: stage 1 dialogue stems (ElevenLabs V3, one job per line, the character's stored `voiceId`, audio tags for performance), stage 2 framed 9:16 stills for every cut (`enhance_prompt` → `generate_shots` on `muse-image`), stage 3 non-speaking Grok Imagine 1.5 clips at 720p from the approved stills, stage 4 VEED Lipsync V2 only for cuts where the speaker's mouth is visible. Quote each stage as one batch, keep takes tied to shot IDs, record rejections and the ledger fields, and reuse style/cast IDs.
